...
class CartSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Cart
        fields = ['user', 'vendor_id', 'category_id', 'vendor', 'category', 'food_item', 'quantity', 'updated_at', 'total_food_items', 'total_price']

    def get_vendor(self, obj):
        return obj.food_item.vendor.vendor_name if obj.food_item and obj.food_item.vendor else None

    def get_category(self, obj):
        return obj.food_item.category.category_name if obj.food_item and obj.food_item.category else None

    def get_food_item(self, obj):
        return obj.food_item.food_title if obj.food_item else None

    def get_total_food_items(self, obj):
        total_food_items = Cart.objects.filter(user=obj.user).aggregate(total_items=Sum('quantity'))['total_items']
    ...
    def create(self, validated_data):
        ...
        if not Category.objects.filter(id=category.id, vendor=vendor).exists():
            logger.error(f"Category ID {category.id} does not belong to Vendor ID {vendor.id}")
            raise serializers.ValidationError("The selected category does not belong to the specified vendor.")
        ...
        if not food_item:
            logger.error(f"No food item found for vendor ID {vendor.id} and category ID {category.id}")
            raise serializers.ValidationError("No food item found for the given vendor and category.")
